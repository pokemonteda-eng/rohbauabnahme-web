...
def test_kunden_crud_flow() -> None:
    ...
    delete_response = client.delete(f"/kunden/{kunde_id}")
    assert delete_response.status_code == 204
    assert delete_response.content == b""

    missing_response = client.get(f"/kunden/{kunde_id}")
...
def test_kunden_error_cases() -> None:
    ...
    assert client.post("/kunden", json=payload).status_code == 201

    duplicate_response = client.post("/kunden", json=payload)
    assert duplicate_response.status_code == 409

    empty_patch_response = client.patch("/kunden/1", json={})
    assert empty_patch_response.status_code == 400

    patch_missing_response = client.patch("/kunden/9999", json={"name": "Neu"})
    assert patch_missing_response.status_code == 404

    delete_missing_response = client.delete("/kunden/9999")
